...
function _node-nvm-switch-cwd-SLOW {
  ...
  local CUR_VER=$(nvm current)
  local ENV_VER=""
  if [[ -f "$PROJECT_ROOT/.nvmrc" ]]; then
    ENV_VER="$(nvm version $(cat "$PROJECT_ROOT/.nvmrc"))"
  fi
  ...
  if [[ "$ENV_VER" != "" ]]; then
    ...
    if [[ "$CUR_VER" != "$ENV_VER" ]]; then
      nvm use && export CD_NODE_USE="$ENV_VER"
    fi
  fi
}
